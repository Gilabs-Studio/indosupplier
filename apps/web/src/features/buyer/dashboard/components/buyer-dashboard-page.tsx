...
export function BuyerDashboardPage() {
    ...
    return (
      <BuyerLayout>
        <div className="text-center py-20 bg-card rounded-xl border border-border">
          <p className="text-destructive font-semibold">Gagal memuat data dashboard. Harap coba lagi.</p>
        </div>
      </BuyerLayout>
    );
  }
  ...
  const stats = [
    { label: t("statsActiveRfqs"), value: String(dashboardData.stats.active_rfqs), desc: "+2 this week", color: "text-primary" },
    { label: t("statsSavedSuppliers"), value: String(dashboardData.stats.saved_suppliers), desc: "Shortlisted", color: "text-success" },
    { label: t("statsNotifications"), value: String(dashboardData.stats.notifications), desc: "3 urgent", color: "text-warning" },
  ];
        ...
        <Card className="border border-border rounded-xl shadow-xs overflow-hidden bg-card">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-border">
              {dashboardData.recent_rfqs.length === 0 ? (
                <p className="text-sm text-muted-foreground py-6 text-center">Belum ada RFQ aktif.</p>
              ) : (
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-semibold text-foreground hover:text-primary transition-colors cursor-pointer">
                          <Link href={`/rfq/${rfq.id}`}>{rfq.product_name}</Link>
                        </h3>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        Dibuat pada {rfq.created_at} • {t("repliesCount", { count: rfq.replies_count })}
                      </p>
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="border border-border rounded-xl shadow-xs overflow-hidden bg-card">
            ...
            <CardContent className="p-4 space-y-4">
              {dashboardData.suggested_suppliers.length === 0 ? (
                <p className="text-sm text-muted-foreground py-6 text-center">Belum ada rekomendasi supplier.</p>
              ) : (
                dashboardData.suggested_suppliers.map((supplier) => (
                  <div key={supplier.id} className="flex items-start justify-between gap-3 p-3 rounded-lg border border-border hover:border-muted-foreground/30 transition-all bg-card">
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5">
                        <h4 className="text-sm font-semibold text-foreground">{supplier.company_name}</h4>
