...
type ProductTypeHandler struct {
	uc usecase.ProductTypeUsecase
}

func NewProductTypeHandler(uc usecase.ProductTypeUsecase) *ProductTypeHandler {
	return &ProductTypeHandler{uc: uc}
}

func (h *ProductTypeHandler) Create(c *gin.Context) {
...
func (h *ProductTypeHandler) GetByID(c *gin.Context) {
	id := c.Param("id")
	result, err := h.uc.GetByID(c.Request.Context(), id)
	if err != nil {
		errors.ErrorResponse(c, "PRODUCT_TYPE_NOT_FOUND", map[string]interface{}{"id": id}, nil)
		return
	}
...
func (h *ProductTypeHandler) Update(c *gin.Context) {
	id := c.Param("id")
	var req dto.UpdateProductTypeRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		if validationErrors, ok := err.(validator.ValidationErrors); ok {
			errors.HandleValidationError(c, validationErrors)
			return
...
func (h *ProductTypeHandler) Delete(c *gin.Context) {
	id := c.Param("id")
	if err := h.uc.Delete(c.Request.Context(), id); err != nil {
		errors.ErrorResponse(c, "PRODUCT_TYPE_NOT_FOUND", map[string]interface{}{"id": id}, nil)
		return
	}
	response.SuccessResponseDeleted(c, "product_type", id, nil)
}
