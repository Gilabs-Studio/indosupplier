...
func (h *PackagingHandler) Create(c *gin.Context) {
	var req dto.CreatePackagingRequest
...
func (h *PackagingHandler) GetByID(c *gin.Context) {
	id := c.Param("id")
	result, err := h.uc.GetByID(c.Request.Context(), id)
	if err != nil {
		errors.ErrorResponse(c, "PACKAGING_NOT_FOUND", map[string]interface{}{"id": id}, nil)
		return
	}
	response.SuccessResponse(c, result, nil)
}

func (h *PackagingHandler) List(c *gin.Context) {
...
func (h *PackagingHandler) Update(c *gin.Context) {
	id := c.Param("id")
	var req dto.UpdatePackagingRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		if validationErrors, ok := err.(validator.ValidationErrors); ok {
			errors.HandleValidationError(c, validationErrors)
			return
...
func (h *PackagingHandler) Delete(c *gin.Context) {
	id := c.Param("id")
	if err := h.uc.Delete(c.Request.Context(), id); err != nil {
		errors.ErrorResponse(c, "PACKAGING_NOT_FOUND", map[string]interface{}{"id": id}, nil)
		return
	}
	response.SuccessResponseDeleted(c, "packaging", id, nil)
}
